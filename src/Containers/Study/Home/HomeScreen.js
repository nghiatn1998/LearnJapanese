...
// Socket connection
import { removeListener, socketConnection, socketCheckConnect } from '../../../Store/Services/SocketAPI'
...
const LogoNotPressed = ({ text, image }) => {
  return (
    <LogoContainer>
...
const HomeScreen = (props) => {
  ...
  useEffect(() => {
    ...
    return () => {
      removeListener(socketCheckConnect)
    }
  }, [])
  ...
  const onPressLogo = (index) => {
    if (index === 1) {
      props.navigation.navigate('Alphabet')
    }
  }
          ...
          <ScrollView style={{height: '100%'}}>
            ...
            >
              {logos.map((logo, index) => {
                if (index === 1) {
                  return (
                    <Logo onPressLogo={() => onPressLogo(index)} key={index} image={logo.image} text={logo.text}/>
                  )
                } else {
                  return (
                    <LogoNotPressed key={index} image={logo.image} text={logo.text}/>
                  )
                }
              })}
            </ScrollView>
