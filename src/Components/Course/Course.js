...
const Course = (props) => {
  const [cardWidth, setCardWidth] = useState(getCourseWidth(Metrics.screenWidth))

  useEffect(() => {
    Dimensions.addEventListener('change', adapterLayout)
  }, [])

  const adapterLayout = dimensions => {
  ...
  return (
    <Container onPress={onPressTopic} style={{width: cardWidth}}>
      <Cover>
